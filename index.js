...
class PieceChain {
	...
	del( offset , length ) {
		...
		//if( startSpan.span === endSpan.span )
	}
...
class PieceChain {
	constructor( str ) {
		this.head = new Span( "" , null , null );
		this.tail = new Span( "" , this.head , null );
		this.head.next = this.tail;
		this.length = 0;
		this._modifyBuffer = [ ]; //the changes
		this._redoBuffer = [ ]; //not implemented
		this._lastAaddedSpan; //not implemented, the last added span 
	}

	insert( offset , str ) {
		let { span , cursor } = this._seek( offset ) ;
		let { newStartSpan , newEndSpan } = this._string_into_spans( str );
		this._modifyBuffer.push( span );

		if( cursor === 0 ) {
			//head
			let newSpan = this._clone( span );
			newStartSpan.previous = newSpan.previous;
			newEndSpan.next = newSpan;
		} else if( cursor === span.length ) {
			//tail
			let newSpan = this._clone( span );
			newStartSpan.previous = newSpan;
			newEndSpan.next = newSpan.next;

		} else {
			
			let { startSpan , endSpan } = this._split( span , cursor );
			endSpan.previous = newSpans;
			while( newSpans.previous ) {
				newSpans = newSpans.previous;
			}
			startSpan = newSpans;
		}
	}
	...
	delete( offset , length ) {
		...
		let { startSpan , startCursor } = this._seek( offset );
		let { endSpan , endCursor } = this._seek( offset + length ); //can speed up by refactoring seek so that it's a function on the sapn
		
		this._modifyBuffer.push( { startSpan , endSpan } );

		let newStartSpan = this._split( startSpan , startCursor );
		let newEndSpan = this._split( endSpan , endCursor );
		startSpan.previous = newStartSpan;
		endSpan.next = newEndSpan;
	}


	_split( span , offset ) {
		let startSpan = new Span( span.content.subString( 0 , offset , span.previous ) );
		let endSpan = new Span( span.content.subString( offset ) , startSpan , span.next);
		startSpan.next = endSpan;
		return{ startSpan , endSpan };
	}
	_clone( span ) {
	...
	_string_into_spans( str  ) {
		...
		let endSpan;
		for( var i = 0 ; i < str.length; i+= SPAN_SIZE ) {
			let span = new Span( str.subString( i , SPAN_SIZE ) );
	...
	_seek( offset ) {
		let combinedOffset = 0;
		let listSpan = this.head;
		while( offset > combinedOffset + listSpan.length ) {
			combinedOffset += listSpan.length;
			listSpan.next();
		}
		...
		return {
			  span : listSpan
			, cursor : offset - ( combinedOffset - listSpan.length )
		}
